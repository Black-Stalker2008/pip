...
def start(update, _):
    update.message.reply_text(f'Привет, {update.effective_user.first_name}, это - бот калькулятор для рациональных чисел. Выберите пожалуйста команду.\n' 'Команда /cancel, чтобы прекратить разговор.\n\n')
    update.message.reply_text('1 - Приступить к вычеслениям ; \n2 - Выйти из калькулятора \n')
    return CHOICE
...
def choice(update, context):
    ...
    if user_choice in '12':
        if user_choice == '1':
            ...
            return RATIONAL_ONE
        if user_choice == '2':
            update.message.reply_text('Спасибо что воспользовались калькулятором, до свидания!')
